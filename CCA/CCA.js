$(document).ready(function () {
	 ...
	 $("#ChosenCardID").trigger('change');
         $(".CardContainer").show();
         $("#ccrawdata").keyup(function(){
                          ValidateInput();
          });
          $("#ccrawdata").keydown(function(){
                         ValidateInput();
          });
    ...
    $("#NewCard_CardNumber").attr("style","border: 1px solid rgb(214, 214, 214); border-image: none; height: 20px; clear: both; float: left;");
    //Customer VAT Reg
...
{
   var curRaw = $("#ccrawdata").val();
curRaw  = curRaw .replace(/[\W_]/g, ""); 
    if (curRaw .length == 6 && isFinite(curRaw ) && !isNaN(parseFloat(curRaw))) {    
      alert("Please enter Credit Card number via SREDkey device");                                
    $("#ccrawdata").val("")    
    return false;
}         
}
