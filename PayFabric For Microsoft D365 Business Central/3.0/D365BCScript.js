...
window.addEventListener('message', function (e) {
    if (e.data.act == 'ProcessTrx') {
        ...
        $(".symbol").text(symbol);
        var symbolWidth = parseInt($(".symbol").width());
        if (symbolWidth > 30) {
...
function customDisplayAdjustment() {
    ...
    $("#summary .total label").after("<div class='symbol'>" + symbol + "</div>");
    var symbolWidth = parseInt($(".symbol").width());
    if (symbolWidth > 30) {
        symbolWidth = symbolWidth + 4;
        $("#TrxAmount").attr("style", "padding-left:" + symbolWidth + "px !important");
...
function removeHtmlElementStyle() {
    ...
    $("#summary .total label").removeClass('bold');
...
window.parent.postMessage("hosted_payment.loaded", "*");